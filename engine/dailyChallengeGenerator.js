...
function isValidAndUnvisited(x, y, rows, cols, visited) {
    return x >= 0 && x < cols && y >= 0 && y < rows && !visited[y][x];
}
// Count unvisited neighbors of a cell
function getUnvisitedNeighborCount(x, y, rows, cols, visited) {
    let count = 0;
    const dirs = [
        { dx: 1, dy: 0 },
        { dx: -1, dy: 0 },
...
function generateDailyChallenge(dateStr) {
    ...
    while (attempt < maxAttempts) {
        ...
        while (unvisitedList.length > 0) {
            ...
            const path = [{ x: startCell.x, y: startCell.y }];
...
function solveAndAssignDirections(paths, rows, cols, rand, dateStr) {
